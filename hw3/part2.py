...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            corners, U2, U1 = cv2.aruco.detectMarkers(frame, arucoDict, parameters=arucoParameters)
            cor = np.round(corners[0][0]).astype(int)
            H1=solve_homography(ref_corns, cor) 

            warping(ref_image, frame, H1, np.min(cor,axis=0)[1], np.max(cor,axis=0)[1], np.min(cor,axis=0)[0], np.max(cor,axis=0)[0], direction='b')
